...
数式が Sarma, Y. and Jammalamadaka, S.(1993) に準じている. よくわからないので一旦保留.
...
# Generate parameter
# もっと綺麗に書ける
alpha <- Bcoef(wind_data_pred) %>% 
            as.data.frame() 
alpha <- alpha$const %>% as.matrix()
beta <- Acoef(wind_data_pred) 
Eps <- summary(wind_data_pred)$covres %>% matrix(nrow = 2)
```

```{r}
wind_data_hat <- CircularRegPred(wind_data$theta,2,alpha,beta,Eps)
```
