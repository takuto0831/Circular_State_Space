...
- Of course, `ggplot` can do it and beautiful...
...
ggplot(x, aes(x =theta)) + 
  ...
  theme_minimal() + labs(title = "Circular histogram")
```
...
- Maximize the sum of log likelihoods at each time point
...
- Autocorrelation plots shows convergence
...
ar2_model <- readRDS("model/circular_AR2.rds") # complie model
fit_ar_7 <- sampling(ar_model,data = d.dat7, iter=2000, chains=4)# sampling by model
```
...
- **If rstan, you can decsribe a distribution that you don't understand**
...
fit_state_obs_5 <- sampling(state_obs_model,data = d.dat5, iter=5000, chains=4) # sampling by model
```
...
- `rstan` uses `ggplot2`, so you change the option
