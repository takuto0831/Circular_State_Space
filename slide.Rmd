...
5. Build a regressive model based on the Circular data
...
- I casually applied for it, but it's the gathering of great peaple...

- Like everyone, I also want to listen to about Mr. Hadley's story, so stay with me for about 15 minutes.
...
## Circular plot by ggplot

- Of course, `ggplot` can do it...
...
- circular standard deviation is defined by $$\sqrt{-2 \log(\mbox{mean resultant length})}$$

```{r}
x <- runif(100,0,pi/2) # data
```
...
## Estimate AR($3$) model parameters
...
## Circular Vector Autoregressive model
