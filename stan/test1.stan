...
parameters{
  vector[2] alpha_0;
  matrix[2,2] alpha_1;
  real<lower=0.001> tau1;
  real<lower=0.001> tau2;
  real<lower=-1,upper=1> rho;
}

transformed parameters{
  cov_matrix[2] sigma;
  sigma[1,1] = tau1; sigma[1,2] = sqrt(tau2)*sqrt(tau1)*rho;
  sigma[2,1] = sqrt(tau2)*sqrt(tau1)*rho; sigma[2,2] = tau2;
}

model{
  alpha_0 ~ multi_normal(rep_vector(0,2),diag_matrix(rep_vector(10^5,2)));
  tau1 ~ inv_gamma(0.01,0.01);
