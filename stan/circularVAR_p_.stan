...
parameters{
  ...
  real<lower=-1,upper=0> phi1;
  real<lower=-1,upper=0> phi2; 
  real<lower=-1,upper=1> phi3; //相関
}
...
model{
  ...
  alpha_0[1] ~ normal(0,10); alpha_0[2] ~ normal(0,10);
  phi1 ~ normal(0,0.5); phi2 ~ normal(0,0.5); phi3 ~ normal(0,0.5);
  for(n in 1+P:N){
