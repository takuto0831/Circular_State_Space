...
pred_value <- function(fit,p){
  pred <- ans_stan_p(fit, P=p, num = length(wind_data$theta_real),data=wind_data$theta_real) # パラメータ推定値を用いて, 予測
  matplot(cbind(wind_data$theta_real[-c(1:p)],pred),type="l")
}
```
...
fit1_4 <- sampling(model4, data = c(d.dat1,lambda=1), iter=1000, chains=1) # Execute model
fit2_4 <- sampling(model4, data = c(d.dat2,lambda=1), iter=1000, chains=1) # Execute model
fit3_4 <- sampling(model4, data = c(d.dat3,lambda=1), iter=3000, chains=1) # Execute model
...
```{r}
# model1
pred_value(fit = fit1_1,p = 1)
pred_value(fit = fit2_1,p = 2)

#model2
pred_value(fit = fit1_2,p = 1)
pred_value(fit = fit2_2,p = 2)

#model3
pred_value(fit = fit1_3,p = 1) # function 内部 変更の必要あり
pred_value(fit = fit2_3,p = 2) # function 内部 変更の必要あり

#model4
pred_value(fit = fit1_4,p = 1)
pred_value(fit = fit2_4,p = 2)
pred_value(fit = fit3_4,p = 3)
```
