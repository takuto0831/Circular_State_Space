...
save(fit6_4, file="fit/circularVAR_6_4.rda")# save fit
save(fit7_4, file="fit/circularVAR_7_4.rda")# save fit
save(fit8_4, file="fit/circularVAR_8_4.rda")# save fit
save(fit9_4, file="fit/circularVAR_9_4.rda")# save fit
...
load("fit/circularVAR_6_4.rda", verbose = TRUE) # load fit
load("fit/circularVAR_7_4.rda", verbose = TRUE) # load fit
load("fit/circularVAR_8_4.rda", verbose = TRUE) # load fit
load("fit/circularVAR_9_4.rda", verbose = TRUE) # load fit
...
fit6_4 # model4, VAR(6)model
fit7_4 # model4, VAR(7)model
fit8_4 # model4, VAR(8)model
fit9_4 # model4, VAR(9)model
...
pred_value(fit = fit6_4,p = 6,dat=wind_data$theta_real)
pred_value(fit = fit7_4,p = 7,dat=wind_data$theta_real)
pred_value(fit = fit8_4,p = 8,dat=wind_data$theta_real)
pred_value(fit = fit9_4,p = 9,dat=wind_data$theta_real)
...
stan_plot(fit1_4,point_est ="median",show_density=T,
          # pars = c("alpha_0","alpha_1","sigma_1"),
          pars =  c("alpha[c,o]","alpha[s,0]","beta[c,1,1]","beta[c,1,2]","beta[s,1,1]","beta[s,1,2]","sigma"),
          ci_level = 0.95, outer_level=1.00, show_outer_line =T) + 
  scale_y_continuous(labels = rev(c(expression(alpha[paste("c,",0)]),expression(alpha[paste("s,",0)]),expression(beta[paste("c,",1,",",1)]),expression(beta[paste("c,",1,",",2)]),expression(beta[paste("s,",1,",",1)]),
                                expression(beta[paste("s,",1,",",2)]),expression(sigma[paste(1,",",1)]),expression(sigma[paste(1,",",2)]),expression(sigma[paste(2,",",1)]),expression(sigma[paste(2,",",2)]))), breaks = 1:10)
stan_ac(fit1_4,pars =  c("alpha[c,o]","alpha[s,0]","beta[c,1,1]","beta[c,1,2]","beta[s,1,1]","beta[s,1,2]","sigma"))
```
...
extract_likelihood(fit6_4) # VAR(6) model
extract_likelihood(fit7_4) # VAR(7) model
extract_likelihood(fit8_4) # VAR(8) model
extract_likelihood(fit9_4) # VAR(9) model
...
extract_log_lik(fit6_4,"log_likelihood") %>% loo::waic() # VAR(6) model
extract_log_lik(fit7_4,"log_likelihood") %>% loo::waic() # VAR(7) model
extract_log_lik(fit8_4,"log_likelihood") %>% loo::waic() # VAR(8) model
extract_log_lik(fit9_4,"log_likelihood") %>% loo::waic() # VAR(9) model
extract_log_lik(fit10_4,"log_likelihood") %>% loo::waic() # VAR(10) model
```


```{r,eval=FALSE}
# algorithm に従って計算, 確認用
...
extract_log_lik(fit1_4,"log_likelihood") %>% waic_ # VAR(1) model
extract_log_lik(fit2_4,"log_likelihood") %>% waic_ # VAR(2) model
extract_log_lik(fit3_4,"log_likelihood") %>% waic_ # VAR(3) model
extract_log_lik(fit4_4,"log_likelihood") %>% waic_ # VAR(4) model
extract_log_lik(fit5_4,"log_likelihood") %>% waic_ # VAR(5) model
extract_log_lik(fit6_4,"log_likelihood") %>% waic_ # VAR(6) model
extract_log_lik(fit7_4,"log_likelihood") %>% waic_ # VAR(7) model
extract_log_lik(fit8_4,"log_likelihood") %>% waic_ # VAR(8) model
extract_log_lik(fit9_4,"log_likelihood") %>% waic_ # VAR(9) model
extract_log_lik(fit10_4,"log_likelihood") %>% waic_ # VAR(10) model
```
...
get_loo <- function(model) {
  log_lik <- extract_log_lik(model,"log_likelihood", merge_chains=FALSE)
  r_eff <- relative_eff(exp(log_lik))
  loo(log_lik, r_eff=r_eff, cores=4)
}
get_loo(fit1_4)
get_loo(fit2_4)
get_loo(fit3_4)
...
aic_(fit1_4,k=1) # VAR(1) model
aic_(fit2_4,k=2) # VAR(2) model
aic_(fit3_4,k=3) # VAR(3) model
aic_(fit4_4,k=4) # VAR(4) model
aic_(fit5_4,k=5) # VAR(5) model
aic_(fit6_4,k=6) # VAR(6) model
aic_(fit7_4,k=7) # VAR(7) model
aic_(fit8_4,k=8) # VAR(8) model
aic_(fit9_4,k=9) # VAR(9) model
aic_(fit10_4,k=10) # VAR(10) model
```
