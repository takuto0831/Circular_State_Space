...
ans_stan_p <- function(fit,P,num,data){
  ...
  for(i in 1:P){
    tmp <- matrix(c(fit_ext$alpha_1[,1,(2*i-1)] %>% mean(),fit_ext$alpha_1[,1,2*i] %>% mean(),
                    fit_ext$alpha_1[,2,(2*i-1)] %>% mean(),fit_ext$alpha_1[,2,2*i] %>% mean()),2,2)
...
```{r set data list, eval=FALSE,include=FALSE}
d.dat1 <- list(N=length(wind_data$theta_real),P=1,theta=wind_data$theta_real,
               para_alpha_1 = 1000, para_alpha_0 = 1000, para_sig = 1000, para_rho = 1000) # for VAR(1)
...
```{r}
pred_value(fit = fit1,p = 1,dat=wind_data$theta_real)
pred_value(fit = fit2,p = 1,dat=wind_data$theta_real)
...
```{r,eval=FALSE,include=FALSE}
shinystan::launch_shinystan(fit1_1)
...
```{r}
log_lik <- c()
a <- extract_log_lik(fit1_1,"log_likelihood")
for( i in 1:dim(a)[1]) {
