...
fit1_4 <- sampling(model4, data = c(d.dat1,lambda=0.5), iter=500, chains=1) # Execute model
fit2_4 <- sampling(model4, data = c(d.dat2,lambda=0.01), iter=1000, chains=1) # Execute model
fit3_4 <- sampling(model4, data = c(d.dat3,lambda=0.5), iter=1000, chains=1) # Execute model
...
# model5
pred_value(fit = fit4_2,p = 4,dat=wind_data$theta_real)
```

# Diagnosis

```{r,eval=FALSE,include=FALSE}
shinystan::launch_shinystan(fit4_4)
```
