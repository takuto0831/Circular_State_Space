...
fit1_2 <- sampling(model2, data = d.dat1, iter=1000, chains=1, thin = 5) # Execute model
...
fit2_5 <- sampling(model5, data = c(d.dat2,lambda=0.01), iter=1000, chains=1) # Execute model
fit3_5 <- sampling(model5, data = c(d.dat3,lambda=0), iter=1000, chains=1) # Execute model
```
...
```{r,eval=FALSE,include=FALSE}
shinystan::launch_shinystan(fit3_2)
...
```{r}
log_lik <- c()
a <- extract_log_lik(fit1_2,"log_likelihood")
for( i in 1:dim(a)[1]) {
