...
```{r not need, eval=FALSE,echo=FALSE}
# 結果のベクトルを抽出 var(1)
# 一般化しkて記述できているので不要
...
ans_stan_p <- function(fit,P,num,data){
  ...
  # Estimate parameter
  len <- dim(mu_hat)[1]; sin.mom <- cos.mom<- 0; pred <- 0
  # 
  for(i in 1:len){
...
model <- stan_model('stan/circularVAR_p.stan')
fit1 <- sampling(model, data = d.dat1, iter=1000, chains=1) # Execute model
fit2 <- sampling(model, data = d.dat2, iter=1000, chains=1) # Execute model 
```
...
```{r}
pred_1 <- ans_stan_p(fit1, P=1, num = length(wind_data$theta_real),data=wind_data$theta_real) # パラメータ推定値を用いて, 予測 
matplot(cbind(wind_data$theta_real[-c(1:1)],c(NA,pred_1)),type="l")
pred_2 <- ans_stan_p(fit2, P=2, num = length(wind_data$theta_real),data=wind_data$theta_real) # パラメータ推定値を用いて, 予測 
matplot(cbind(wind_data$theta_real[-c(1:2)],c(NA,pred_2)),type="l")
```
...
loo::waic(log_lik1)
log_lik2 <- extract_log_lik(fit2,"log_likelihood")
loo::waic(log_lik2)
```
